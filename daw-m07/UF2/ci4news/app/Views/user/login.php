        ...
        <div class="row box-shadow d-flex justify-content-center">
            <div class="col-6 bg-body px-5 pb-5 rounded">
                ...
                <div class="d-flex text-end close-container mb-5">
                    <a id="close-btn" href="<?= base_url() ?>"><i
                                class="fa-regular fa-circle-xmark fa-xl text-dark"></i></a>
                </div>
                ...
                <form action="<?=base_url('login')?>" method="POST">
                    <div class="row mb-3">
                        <label for="nick" class="text-uppercase mb-1">Nom d'usuari</label>
                        <input type="text" name="nick" class="form-control" name="nick" id="nick" placeholder="Usuari"
                               value="<?= (!empty($_COOKIE['username'])) ? $_COOKIE['username'] : '' ?>" required/>
                    </div>
                    <div class="row mb-3">
                        <label for="password" class="text-uppercase mb-1">Contrasenya</label>
                        <input type="password" name="contrasenya" class="form-control" id="password"
                               placeholder="Contrasenya" required/>
                    </div>
                    <div class="mb-3 form-check">
                        <label for="remember" class="form-label">Recorda el nom d'usuari</label>
                        <input type="checkbox" class="form-check-input" id="remember"
                               name="connected" <?= !empty($_COOKIE['remember']) ? 'checked' : '' ?> />
                    </div>
